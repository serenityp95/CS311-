  ...
  if (V != NULL)
    {
      ...
      cout << "Balance: " << V->Balance << endl;// ** display V's element, height and balance and do endl; 
...
void BST::DeleteVertex(el_t E)
{
  ...
  { cout << "... deleting the root with just one child" << endl; 
    ...
    else if (Root->Right != NULL)// ** and it has only the right subtree
    { 
      delete Root;
      Root = Root->Right;
      Root->Up = NULL;
...
void BST::remove(Vertex *V, Vertex *P)
{
  if (V->Right == NULL && V->Left == NULL) // ** if V is a leaf (case 1)
  ...
  else if (V->Left == NULL && V->Right != NULL) // ** if V has just the right child so bypass V (case 3)
   ...
   else // V has two children (case 4)
