...
{
  int slot = hash(element.getkey()); // hash's key to get slot in table

  int position = table[slot].search(element); //finds position of slot based on element

  table[slot].deleteIth(position, element); // calls llist's deleteIth function to delete element
  
  return slot; // returns slot number
}
