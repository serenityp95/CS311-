...
{ 
 ...
 public:  // available to the client
  ...
  //          otherwise, throws an exception - Underflow
  // PARAMETER: provide variable to receive top element (by ref)
  void topElem(el_t&);

  //PURPOSE: if stack is empty, return true; otherwise return false
  bool isEmpty();

  //PURPOSE: if stack is full, return true; otherwise return false
  bool isFull();

  //PURPOSE: display all elements vertically in the stack
  void displayAll();

  //PURPOSE: clear all elements in the stack
  void clearIt();
